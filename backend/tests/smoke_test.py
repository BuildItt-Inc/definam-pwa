...
async def run_smoke_test():
    print("[START] Starting DefinAm Backend Smoke Test...")
    ...
    try:
        ...
        async with session_factory() as session:
            ...
            await session.commit()
            print(f"[SUCCESS] Setup individual access code: {test_code}")
        ...
        print(f"[INFO] Generated Access Token: {login_res['access_token'][:30]}...")
        print(f"[INFO] Generated Refresh Token: {login_res['refresh_token'][:30]}...")

        # 5. Test Token Refresh Lifecycle
        print("[INFO] Testing refresh token rotation lifecycle...")
        ...
        print("[SUCCESS] Refresh and token rotation successful!")

        print(
            "\n[SUCCESS] SMOKE TEST COMPLETED SUCCESSFULLY! All components are fully operational."
        )

    finally:
        # 6. Database Cleanup
        print("[INFO] Cleaning up smoke test database records...")
        async with session_factory() as session:
            ...
            await session.commit()
        print("[INFO] Database successfully cleaned.")
