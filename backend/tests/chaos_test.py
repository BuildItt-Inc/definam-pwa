...
async def run_chaos_test():
    ...
    async with session_factory() as session:
        ...
        # Access Codes
        code_a1 = AccessCode(code="CHAOS-SCHA-1111", type="org", status="pending", school_id=school_a.id)
        code_a2 = AccessCode(code="CHAOS-SCHA-2222", type="org", status="pending", school_id=school_a.id)
        code_b1 = AccessCode(code="CHAOS-SCHB-1111", type="org", status="pending", school_id=school_b.id)
        code_ind = AccessCode(code="CHAOS-INDV-9999", type="individual", status="pending")
        session.add_all([code_a1, code_a2, code_b1, code_ind])
        
        # Users
        # OrgAdmin A (role: admin, belongs to School A)
        admin_a = User(username="chaos_admin_a", password_hash="dummy", role="admin", org_id=school_a.id)
        # OrgAdmin B (role: admin, belongs to School B)
        admin_b = User(username="chaos_admin_b", password_hash="dummy", role="admin", org_id=school_b.id)
        # Student A (role: student_org, registered under School A)
        student_a = User(username="chaos_student_a", password_hash="dummy", role="student_org", org_id=school_a.id)
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
        ...
        async with session_factory() as session:
            ...
            session.add(chap)
            await session.flush()
            topic = Topic(chapter_id=chap.id, title="Simple Equations", status="published")
            session.add(topic)
            await session.commit()
            topic_uuid = topic.id
